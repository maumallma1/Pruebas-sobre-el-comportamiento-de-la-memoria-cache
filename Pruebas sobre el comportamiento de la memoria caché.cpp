...
int main() {
    ...
    int blockSize = 5; // Tamaño del bloque, puede ajustarse para optimización
        ...
        for (int iBlock = 0; iBlock < N; iBlock += blockSize) {
            for (int jBlock = 0; jBlock < N; jBlock += blockSize) {
                for (int kBlock = 0; kBlock < N; kBlock += blockSize) {
                    // Multiplicar los bloques A[iBlock:kBlock] por B[kBlock:jBlock] y sumar al bloque C[iBlock:jBlock]
